...
AUTO_ADMIN_EMAIL = "Admin@Admin.com"
...
/* Uploaded regular-agent dashboard header */
.dashboard-blue-header {
  position:relative;
  z-index:1;
  background:linear-gradient(135deg,#0b3f63 0%,#145f88 100%);
  color:#fff;
  border-radius:12px 12px 6px 6px;
  padding:10px 16px;
  margin:0 0 -58px 0;
  min-height:72px;
  box-shadow:0 2px 5px rgba(13,43,67,.14);
}
...
/* The Streamlit popover is pulled upward so its trigger sits inside the
   dashboard's blue header rather than above it. */
.dashboard-blue-header + [data-testid="stHorizontalBlock"] {
  position:relative;
  z-index:4;
  min-height:58px;
...
.dashboard-blue-header + [data-testid="stHorizontalBlock"] [data-testid="column"]:first-child {
  pointer-events:none;
}
.dashboard-blue-header + [data-testid="stHorizontalBlock"] [data-testid="column"]:last-child {
  pointer-events:auto;
}
.dashboard-blue-header + [data-testid="stHorizontalBlock"] button {
...
.dashboard-blue-header + [data-testid="stHorizontalBlock"] button:hover {
...
body:has(.auth-page-marker) [data-testid="stAppViewContainer"] {
  background:
    radial-gradient(circle at 15% 8%, rgba(74,148,208,.78), transparent 36%),
    linear-gradient(135deg,#5c98c4 0%,#326e9f 55%,#285f8e 100%) !important;
}
body:has(.auth-page-marker) [data-testid="stHeader"] {
  display:none !important;
}
body:has(.auth-page-marker) .auth-page-marker {
  height:0 !important;
  margin:0 !important;
  padding:0 !important;
}
...
body:has(.auth-page-marker) [data-testid="stHorizontalBlock"] {
...
body:has(.auth-page-marker) .block-container {
...
body:has(.auth-page-marker) [data-testid="stVerticalBlock"] {
  gap:0 !important;
}
body:has(.auth-page-marker) [data-testid="column"] {
  padding:0 !important;
}
.auth-brand-panel {
  position:relative;
...
body:has(.auth-page-marker) [data-testid="stTextInput"] label,
body:has(.auth-page-marker) [data-testid="stDateInput"] label {
  color:#17283e !important;
  font-size:12px !important;
  font-weight:750 !important;
}
body:has(.auth-page-marker) [data-testid="stTextInput"] input,
body:has(.auth-page-marker) [data-testid="stDateInput"] input {
...
body:has(.auth-page-marker) [data-testid="stTextInput"] input:focus,
body:has(.auth-page-marker) [data-testid="stDateInput"] input:focus {
  border-color:#1876c5 !important;
  box-shadow:0 0 0 2px rgba(24,118,197,.10) !important;
}
body:has(.auth-page-marker) [data-testid="stTextInput"] {
  margin-bottom:7px !important;
}
body:has(.auth-page-marker) [data-testid="stCheckbox"] {
  margin-top:2px !important;
}
body:has(.auth-page-marker) [data-testid="stCheckbox"] label p {
  font-size:12px !important;
  color:#22354a !important;
}
body:has(.auth-page-marker) [data-testid="stFormSubmitButton"] button,
body:has(.auth-page-marker) .stButton > button {
...
body:has(.auth-page-marker) [data-testid="stFormSubmitButton"] button:hover,
body:has(.auth-page-marker) .stButton > button:hover {
  background:#096bb3 !important;
  border-color:#096bb3 !important;
}
body:has(.auth-page-marker) .auth-forgot {
  text-align:right;
...
body:has(.auth-page-marker) button[key="go_signup"] {
...
body:has(.auth-page-marker) .signup-heading + [data-testid="stForm"] {
  padding-bottom:0 !important;
}
body:has(.auth-page-marker) [data-testid="stForm"] [data-testid="stTextArea"] {
  margin-bottom:0 !important;
}
.auth-bottom-login {
  text-align:center;
...
body:has(.auth-page-marker) button[key="go_signin"] {
...
def init_db():
    ...
    # Guarantee the requested owner remains the only automatic super admin.
    # The password is stored only as a secure hash.
    admin_email = normalize_email(AUTO_ADMIN_EMAIL)

    # Retire the previous username-only bootstrap account. Do not delete it;
    # simply remove its admin privileges so only Admin@Admin.com is automatic.
    user_collection.update_many(
        {"email": "admin"},
        {"$set": {"role": "regular", "updated_at": now()}}
    )
...
def sign_up(data):
    ...
    try:
        ...
        # Defensive write: guarantees the persisted signup record carries the
        # requested discriminator even if the schema evolves later.
        db[USER_COLLECTION].update_one(
            {"_id": result.inserted_id},
            {"$set": {"type": "roster_list", "updated_at": now()}}
        )
        get_user.clear()
    ...
    return list(db[USER_COLLECTION].find(
        {
            ...
            "active": True,
            "kicked": {"$ne": True},
            # Missing AUX is treated as unavailable rather than silently
            # eligible for auto-assignment. New signups always have AUX,
            # but this also protects legacy roster records.
            "aux": {"$exists": True, "$nin": list(excluded)},
        },
        {
...
def login_screen():
    ...
    """
    # Marker used by CSS so the authentication page can remove Streamlit's
    # default top whitespace without affecting the dashboard.
    ...
    with right:
        if st.session_state.auth_action == "Sign in":
            ...
            with st.form("login_form", clear_on_submit=False):
                email = st.text_input(
                    "HPE Email Address",
                    placeholder="Admin@Admin.com or name@hpe.com",
                    key="login_email",
                )
...
def save_aux_realtime(user_email):
    aux = st.session_state.get("profile_aux")
    """Persist AUX immediately when the profile selector changes.

    No Save button is required. The assignment engine reads this same
    MongoDB field, so an agent becoming unavailable is excluded on the
    next assignment check.
    """
    if not aux:
        return
    ...
    try:
        ...
        pass
    audit("aux_changed", email, details={"aux": aux, "source": "profile_realtime"})
...
def render_profile_menu(user):
    ...
    with st.popover(f"👤 {name} ▾", use_container_width=True):
        ...
        # AUX is persisted immediately when changed. There is deliberately
        # no Save button: the assignment engine reads the same DB value.
        st.selectbox(
            "AUX",
            aux_options,
            index=aux_options.index(default_aux),
            key="profile_aux",
            on_change=save_aux_realtime,
            args=(user["email"],),
        )
        st.caption("AUX changes are saved automatically.")
...
def render_dashboard(user):
    ...
    # Dashboard header matches the uploaded reference: one blue title bar,
    # with the profile control visually inside that same blue area.
    st.markdown(
        f"""
        <div class="dashboard-blue-header">
          <div>
            <div class="dashboard-blue-title">{"TEAM OVERVIEW" if is_admin else "1. DASHBOARD (AGENT)"}</div>
            <div class="dashboard-blue-subtitle">{"Overall queue status, agent stats and alerts" if is_admin else "Overview of assigned cases, quick stats and alerts"}</div>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    # Pull the real profile popover into the right side of the blue header.
    _header_spacer, _header_profile = st.columns([8.7, 1.3])
    with _header_profile:
...
def realtime_tick(user):
    ...
    try:
        ...
        live_user = db[USER_COLLECTION].find_one(
            {"email": normalize_email(user["email"])},
            {"aux": 1},
        ) or {}
        live_aux = live_user.get("aux", user.get("aux", "Busy - Away"))
        st.caption(
            f"🟢 Live · {now().strftime('%I:%M:%S %p')} · "
            f"AUX: {live_aux} · {active} active queue cases · {unread} unread alerts"
        )
    except Exception as exc:
...
def main():
    ...
    # Dashboard renders its own profile control inside the blue header.
    # Other pages retain the profile control in their page header.
    if page != "Dashboard":
